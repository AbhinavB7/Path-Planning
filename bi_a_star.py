...
goal = None
start_point = None
canvas = np.ones((3000, 6000, 3), dtype=np.uint8 )
...
# def Obstacles(obs):
#     center_1 = (1120, y - 2425)
#     center_2 = (2630, y - 900)
#     center_3 = (4450, y - 2200)

#     cv2.circle(obs, center_1, 400, blue, -1)
#     cv2.circle(obs, center_2, 700, blue, -1)
#     cv2.circle(obs, center_3, 375, blue, -1)
...
cv2.circle(canvas, center_1, 400, blue, -1)
cv2.circle(canvas, center_2, 700, blue, -1)
cv2.circle(canvas, center_3, 375, blue, -1)
...
def obstacle_space(point):
    if is_in_circle(center_1, 400, point):
        return True
    if is_in_circle(center_2, 700, point):
        return True
    if is_in_circle(center_3, 375, point):
        return True
    return False

# Checking if the point can move
def canMove(point):
    if point[0] < 1 or point[0] > 6000 or point[1] < 1 or point[1] > 3000:
        return False 
...
def draw_arrow(path):
    ...
    for i in range(len(path) - 1):
        start_node = path[i]
        end_node = path[i + 1]
        cv2.arrowedLine(canvas, start_node, end_node, (0, 255, 0), 7, tipLength= 0.5)
        # video_writer.write(canvas)
